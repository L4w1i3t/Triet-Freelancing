...
function buildCustomerHtml(orderData, config) {
  ...
  return buildEmailShell(
    ...
    `
      ...
      ${buildItemsHtmlBlock(orderData, config.siteUrl)}
      ${buildDownloadsHtmlBlock(downloads)}
      ${buildTotalsHtmlBlock(orderData)}
    `,
  );
}

function buildDownloadsHtmlBlock(downloads) {
  if (!Array.isArray(downloads) || downloads.length === 0) return "";

  const links = downloads
    .filter((item) => item.downloadUrl && item.title)
    .map(
      (item) =>
...
function getCustomizationLines(item) {
  ...
  if (customization.fontLabel) {
    lines.push(`Font: ${cleanText(customization.fontLabel)}`);
  }
  if (customization.salutation) {
    lines.push(`Salutation: ${cleanText(customization.salutation)}`);
  }
  ...
  if (customization.signoff) {
    lines.push(`Signoff: ${cleanText(customization.signoff)}`);
  }
  if (customization.signature) {
    lines.push(`Name: ${cleanText(customization.signature)}`);
  }
  if (customization.hasPhoto) {
    lines.push("Photo: included");
  }
  if (customization.outputFormat) {
    lines.push(`Output: ${cleanText(customization.outputFormat)}`);
  }
...
function getDownloadTitle(item) {
  return cleanText(
    item?.product?.downloadLabel || getItemName(item) || "Download",
  );
}
